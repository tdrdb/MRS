...
public class UserPanel extends JPanel {
  ...
  UserPanel(List<IUser> userList, List<Movie> movieList, DataSet dataSet) {
	users = new UserMgmt(userList, dataSet);
	initComponents();
  }
...
/**
  ...
  private void editUsersButtonActionPerformed(ActionEvent evt) {
    ...
    surNameUsersTextField.setText(users.getUserAt(rowCount).getName());
    firstNameUsersTextField.setText(users.getUserAt(rowCount).getFirstName());
  ...
  private void deleteUsersButtonActionPerformed(ActionEvent evt) {
    ...
    if (!users.removeUserAt(rowCount)) {
      IUser user = users.getUserAt(rowCount);
  ...
  private void saveUsersButtonActionPerformed(ActionEvent evt) {
    ...
    if (users.saveUser(surName, firstName, rowCount)) {
  ...
  private TableModel createTableModel() {
    return new UserTableModel(users.getUserListAsObject());
  }
